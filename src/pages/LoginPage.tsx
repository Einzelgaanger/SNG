...
import { ArrowRight, Globe2, Shield, Sparkles, Users, Zap } from "lucide-react";
    ...
    <div className="flex min-h-screen bg-background">
      ...
      <div className="relative hidden flex-1 overflow-hidden lg:flex">
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          <svg className="absolute -left-16 bottom-16 h-[350px] w-[350px] text-accent/[0.06]" viewBox="0 0 350 350" fill="none">
        ...
        <div className="relative z-10 flex flex-1 flex-col justify-between p-12 xl:p-16">
          ...
          <div className="fade-in-slow max-w-lg space-y-8">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={f.label} className="rounded-xl border border-border/60 bg-card p-4 shadow-sm">
          ...
          <div className="flex items-center gap-6 text-xs text-muted-foreground">
            ...
            <span className="flex items-center gap-1.5"><Zap className="h-3.5 w-3.5 text-primary/60" /> Real-time intelligence</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex w-full flex-col items-center justify-center px-6 py-12 lg:w-[480px] lg:shrink-0 xl:w-[520px]">
        ...
        <div className="fade-in w-full max-w-sm space-y-8">
          ...
          <form className="space-y-3" onSubmit={handleEmailAuth}>
            {mode === "signup" && (
              <Input placeholder="Full name" value={displayName} onChange={(e) => setDisplayName(e.target.value)} className="h-11" />
            )}
            <Input type="email" placeholder="Email address" value={email} onChange={(e) => setEmail(e.target.value)} className="h-11" />
            {mode !== "forgot" && (
              <Input type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} className="h-11" />
            )}
          ...
          <div className="flex items-center justify-center gap-4 text-sm">
            {mode !== "signin" && (
              <button type="button" className="text-muted-foreground transition hover:text-primary" onClick={() => setMode("signin")}>Sign in</button>
            )}
            {mode !== "signup" && (
              <button type="button" className="text-muted-foreground transition hover:text-primary" onClick={() => setMode("signup")}>Create account</button>
            )}
            {mode !== "forgot" && (
              <button type="button" className="text-muted-foreground transition hover:text-primary" onClick={() => setMode("forgot")}>Forgot password?</button>
            )}
          </div>
